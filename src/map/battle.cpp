	...
	switch(skill_id) {
		...
		case GC_CROSSRIPPERSLASHER:
			...
			if (sc && sc->data[SC_ROLLINGCUTTER])
				skillratio += sc->data[SC_ROLLINGCUTTER]->val1 * 200 + (status_get_agi(src) * 3);
			break;
		case GC_DARKCROW:
			skillratio += 100 * (skill_lv - 1);
			break;
		...
		if (sc->data[SC_EDP]) {
			switch(skill_id) {
				...
				default: // fall through to apply EDP bonuses
					...
					ATK_RATE(wd->weaponAtk, wd->weaponAtk2, 100 + (sc->data[SC_EDP]->val1 * 100));
					ATK_RATE(wd->equipAtk, wd->equipAtk2, 100 + (sc->data[SC_EDP]->val1 * 80));
					break;
#else
				default:
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "quest_area_size",                    &battle_config.quest_area_size,                 14,     0,      INT_MAX,        },
	{ "quest_eden_area_size",               &battle_config.quest_eden_area_size,            700,    0,      INT_MAX,        },
